...
int reg_proc(char *pass_arg[]);
int is_back_job(char *pass_arg[], int num_arg);
int is_com_pipe(char symbol[]);
int is_IO_redirect(char symbol[]);
int exe_back_job(char *pass_arg[], int num_arg);
int exe_com_pipe(char *pass_arg[], char *pass_arg2[]);
int exe_IO_redirect(char *pass_arg[], char *pass_arg2[], char symbol[]);
      ...
      if(num_arg != -2)
        {
          ...
          else if(is_back_job(args, num_arg))
            {
              exe_back_job(args, num_arg);
            }
          else if(is_com_pipe(symbol))
            {
              exe_com_pipe(args, args2);
            }
          else if(is_IO_redirect(symbol))
            {
              exe_IO_redirect(args, args2, symbol);
            }
          else
            {
              reg_proc(args);
...
/*
   Purpose: execute a background job

   Return value: 1 upon successful execution of child, -1 otherwise
 */
int exe_back_job(char *pass_arg[], int num_arg)
{
...
/*
   Purpose: execute command pipeline
   Details: This function will first set up a pipe for the tow child processes, it will then create a child and specify that childs
   output using the dup2 call. It will then pass the input in pass_arg[] to be executed. Upon successful execution the function will
   fork to create a second child, this child has its input specified by the dup2 call and will recieve any input sent down the pipe
   from the first child. It will then execute whatever is in pass_arg2[].
   Return value: 1 upon successful execution of child, -1 otherwise
 */
...
/*
   ...
   Return value: 1 if '|' is in input and 0 otherwise
*/
...
/*
   Purpose: To identify if the '<' or '>' symbol is in input
...
/*
   Purpose: compares 2 strings to see if they are equal

   Return value: 0 means strings are equal anything else means not equal
*/
